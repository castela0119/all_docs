...
const getAccessToken = async (code) => {
  ...
  fetch(tokenUrl, {
    ...
    .then((data) => {
      ...
      sendAccessTokenToBackend(data.access_token)
    })
    .catch((error) => {
...
const sendAccessTokenToBackend = async (accessToken) => {
  try {
    ...
    if (response.ok) {
      ...
      // 추가 처리 (예: 상태 업데이트)
      // 사용자 정보를 쿠키에 저장 (예: 1시간 유효)
      document.cookie = `userInfo=${JSON.stringify(userInfo)}; path=/; max-age=${60 * 60}; secure; samesite=strict`
      document.cookie = `accessToken=${accessToken}; path=/; max-age=${60 * 60}; secure; samesite=strict`

      // 홈으로 이동
      window.location.href = '/' // 또는 원하는 홈 경로로 변경
    } else {
